...
export default function MenuPage() {
  ...
  return (
    <div className="min-h-screen bg-zayko-900 pb-28 md:pb-24">
      ...
      <div className="bg-zayko-800/80 backdrop-blur-xl border-b border-white/[0.06] px-4 py-3 sm:px-6 sm:py-4 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] sm:text-xs text-zayko-400 flex items-center gap-1.5">
              <span className={`w-1.5 h-1.5 rounded-full ${isCanteenOpen ? "bg-emerald-400 animate-pulse" : "bg-red-400"}`} />
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link href="/wallet" className="flex items-center gap-2 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-gold-500/10 hover:border-gold-500/20 transition-all group" title="Wallet">
      ...
      <div className="px-4 sm:px-6 max-w-7xl mx-auto mt-4 mb-2">
        <div className="grid grid-cols-3 gap-2.5 sm:gap-4">
          {[
            ...
            { href: "/dashboard/suggest-item", icon: "💡", label: "Suggest", color: "bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border-amber-500/20" },
            ...
            <Link key={item.href} href={item.href}
              className={`flex flex-col items-center gap-1.5 py-3 sm:py-4 px-2 rounded-2xl border transition-all hover:scale-[1.02] active:scale-[0.98] ${item.color}`}>
      ...
      {availableItems.length > 0 && !search && category === "all" && (
        <div className="mt-8 mb-4">
          <div className="px-4 sm:px-6 max-w-7xl mx-auto flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] sm:text-xs font-bold text-gold-500 bg-gold-500/10 px-2 py-0.5 rounded-full border border-gold-500/20">POPULAR</span>
          </div>
          <div className="flex overflow-x-auto no-scrollbar gap-4 px-4 sm:px-6 pb-4">
            {availableItems.slice(0, 5).map((item, idx) => (
              <div key={`trending-${item.id}`} className="min-w-[200px] sm:min-w-[240px] shrink-0">
                <MenuCard {...item} />
              </div>
            ))}
          </div>
      ...
      <div className="sticky top-[56px] sm:top-[72px] z-30 bg-zayko-900/95 backdrop-blur-xl border-b border-white/[0.04] shadow-xl shadow-black/20">
        <div className="px-4 sm:px-6 max-w-7xl mx-auto py-3 space-y-3">
          ...
          <div className="relative group">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-3 rounded-2xl bg-white/[0.06] border border-white/[0.08] text-white placeholder:text-zayko-500 focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400/30 text-sm transition-all focus:bg-white/[0.08]"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-1 px-1 pb-0.5">
            <button onClick={() => setCategory("all")}
              className={`px-3.5 py-1.5 rounded-full text-xs sm:text-sm font-semibold whitespace-nowrap transition-all shrink-0 ${category === "all"
                ? "bg-gold-500 text-zayko-900 shadow-md shadow-gold-500/20"
                : "bg-white/[0.06] text-zayko-400 border border-white/[0.08] active:scale-95"}`}>
              All
            </button>
            {categories.map((cat) => (
          ...
          <>
            ...
            {availableItems.length > 0 && (
              <section>
                <div className="flex items-center gap-2.5 mb-4">
                  <div className="w-1 h-6 sm:h-7 bg-emerald-500 rounded-full" />
            ...
            {showUnavailable && unavailableItems.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-5 opacity-75">
      ...
      <AnimatePresence>
        {itemCount > 0 && (
          <motion.div
            ...
            className="fixed bottom-16 md:bottom-4 left-3 right-3 sm:left-auto sm:right-6 sm:w-auto z-40"
          >
            <Link
              href="/cart"
              className="flex items-center justify-between gap-4 sm:gap-6 bg-gradient-to-r from-gold-500 to-gold-400 text-zayko-900 px-4 sm:px-6 py-3 sm:py-4 rounded-2xl shadow-[0_8px_32px_rgba(251,191,36,0.3)] hover:shadow-[0_12px_40px_rgba(251,191,36,0.4)] transition-all active:scale-[0.97]"
            >
              <div className="flex items-center gap-2.5">
                <span className="bg-zayko-900/20 w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-sm sm:text-lg">
                  🛒
                </span>
                <div>
